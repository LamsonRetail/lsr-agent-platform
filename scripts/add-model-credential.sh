...
#
# Dùng:
#   ./add-model-credential.sh <id> <subscription|api_key> [priority] [owner_email]
...
curl -s -X POST "http://localhost:8090/v1/model-auth/credentials" \
  ...
  -d "{\"id\":\"$ID\",\"kind\":\"$KIND\",\"secret_ref\":\"$REF\",\"priority\":$PRIO,\"owner_email\":\"$OWNER\"}"
echo
echo "✓ đã đăng ký ref '$REF' vào pool (priority=$PRIO). Kiểm tra: GET /v1/model-auth/credentials"
