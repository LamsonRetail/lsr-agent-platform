...
def answer(prompt, ctx):
    ...
    (version publish trên Console no-code, gồm cả rule "chỉ trả lời trong phạm vi Sourcing,
    không lộ brain sang dự án con khác"), không hard-code ở đây.
    """
...
def draft_minutes(transcript, ctx):
    """Soạn biên bản nháp (key_points + decisions) từ transcript cuộc họp sourcing."""
    prompt = ("Tóm tắt transcript cuộc họp Sourcing thành biên bản gồm "
...
def handle(job):
    ...
    if payload.get("kind") == "transcript":
        # B. Được add vào họp sourcing → soạn nháp, xin chủ trì xác nhận, CHƯA tạo task.
        ...
        # Đã confirm → lưu biên bản vào brain riêng của AG-SOURCING + đề nghị tạo task qua
        # skill Lark Task (endpoint /v1/self/brain/* ghi brain riêng — xem docs/TESTCASES.md#BR.5).
        if not DRY_RUN:
            api("POST", "/v1/self/brain/items", {"title": f"Biên bản Sourcing ({sid})",
